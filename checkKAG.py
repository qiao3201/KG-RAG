...
def format_result_for_output(result: Dict[str, Any], question_id: int) -> str:
    """格式化结果为可读的文本"""
    output = []
    output.append(f"{'='*80}")
    output.append(f"问题 ID: {question_id}")
    output.append(f"问题: {result['question']}")
    output.append(f"{'='*80}")
    
    # 实体部分
    output.append(f"\n【相关实体】 (共 {len(result['entities'])} 个)")
    for i, entity in enumerate(result['entities'], 1):
        output.append(f"  {i}. {entity['name']} (类型: {entity['type']}, 主题: {entity['topic']}, 相似度: {entity['score']:.4f})")
    
    # 三元组部分
    output.append(f"\n【相关三元组】 (共 {len(result['triplets'])} 个)")
    for i, triplet in enumerate(result['triplets'], 1):
        output.append(f"  {i}. {triplet['source']} --[{triplet['relation']}]--> {triplet['target']}")
    
    # 文本块部分
    output.append(f"\n【相关文本块】 (共 {len(result['chunks'])} 个)")
    for i, chunk in enumerate(result['chunks'], 1):
        output.append(f"\n  --- 文本块{i} (来源: 文献{chunk['source_paper']}) ---")
...
def main():
    ...
    # 打开输出文件
    with open('outcome.txt', 'w', encoding='utf-8') as out_file:
        ...
        for i in range(1, total_questions + 1):
            ...
            if question_id in questions:
                ...
                # 格式化结果
                formatted_result = format_result_for_output(result, i)
    ...
    print(f"\n{'='*60}")
    print(f"处理完成！结果已保存到 outcome.txt")
